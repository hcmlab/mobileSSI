...
bool browser(void *arg) {
	...
	Browser *browser = ssi_create (Browser, 0, true);
	browser->getOptions()->setTitle("Browser");
	browser->getOptions()->setPos(400, 0, 400, 600);
...
bool stimuli(void *arg) {
	...
	browser = ssi_create(Browser, 0, true);
	browser->getOptions()->setTitle("Browser");
	browser->getOptions()->setPos(400, 300, 400, 300);
	board->RegisterListener(*browser, stimuli->getEventAddress());
...
bool images(void *arg) {
	...
	viewer->getOptions()->setPos(400, 0, 400, 300);
