...
void Stimuli::listen_enter() {
    ...
    if (_options.annopath[0] != '\0') {
        _anno_writer = new old::FileAnnotationWriter(_options.annopath);
    }
...
void Stimuli::loadHelp(ssi_size_t n, ssi_char_t **names, ssi_char_t **labels, bool randomize, bool insertBlank) {
    ...
    delete[] indices;
    _n_names = ssi_size_t(_names.size());
}
        ...
        if (n > 0) {
            ...
            for (ssi_size_t i = 0; i < n; i++) {
                if (_options.randomize) {
                    ...
                    ssi_size_t n_indices = ssi_size_t(names_list_tmp.size());
                    if (n_indices > 0) {
                        ssi_size_t *indices = new ssi_size_t[n_indices];
                        for (ssi_size_t j = 0; j < n_indices; j++) {
                            indices[j] = j;
                        }
                        Random::Shuffle(n_indices, indices);
                        for (ssi_size_t j = 0; j < n_indices; j++) {
            ...
            delete[] folders;
            ssi_size_t n = ssi_size_t(names_list.size());
    ...
    if (_options.names[0] != '\0') {
        ...
        if (n > 0) {
            ...
            if (_options.labels[0] != '\0') {
                ...
                if (n_labels != n) {
                    ssi_wrn("#names ('%u') != #labels ('%u')", ssi_size_t(_names.size()), n)
                } else {
                    labels = new ssi_char_t *[n];
