...
#ifndef SSI_MODEL_TENSORFLOW_H
#define SSI_MODEL_TENSORFLOW_H
...
#define SSI_TENSORFLOW_MAXHANDLER 5

namespace ssi {

class Trainer;

class Tensorflow : public IConsumer {
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "Tensorflow"; };
	static IObject *Create (const ssi_char_t *file) { return new Tensorflow (file); };
	~Tensorflow ();

	Options *getOptions () { return &_options; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Applies Tensorflow to a stream and outputs result as an event."; };
...
void split(const std::string &s, char delim, Out result);
	std::vector<std::vector<float>> load_input_data(std::string file_name);
	int callTensorflow();

protected:

	Tensorflow (const ssi_char_t *file = 0);
	Tensorflow::Options _options;
	ssi_char_t *_file;
