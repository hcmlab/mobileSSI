...
#ifndef SSI_VECTORFUSION__H
#define SSI_VECTORFUSION__H
...
class VectorFusion : public IObject {
...
public:
	...
	public:
		...
		Options (): dimension(0), update_ms(1000), decay_type (EVector::DECAY_TYPE_HYP), fusionspeed(0.1f), eventspeed (0.1f), gradient(0.5f), threshold(0.1f), accelerate (false), decay_weights (false), negative (true), print(false), paint(false), paint_events(true) {
			...
			addOption("decay_weights", &decay_weights, 1, SSI_BOOL, "decay event weights over time");
...
public: 	
	...
	static const ssi_char_t *GetCreateName () { return "VectorFusion"; };
	static IObject *Create (const ssi_char_t *file) { return new VectorFusion (file); };
	~VectorFusion ();
...
#ifndef HEADLESS
        ICanvasClient* getPlot(){ return (ssi::ICanvasClient*)_plot; };
#endif
...
protected:
	...
	VectorFusion (const ssi_char_t *file = 0);
	Options _options;
	ssi_char_t *_file;
