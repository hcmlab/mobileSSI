...
namespace ssi {
	...
	void ModelTools::LoadAnnotation(old::Annotation &annotation, File &file, char* tier_id) {
	...
	void ModelTools::LoadAnnotation(old::Annotation &anno,
	...
	void ModelTools::SaveAnnotation(old::Annotation &anno,
		File &file) {
	...
	void ModelTools::ConvertToContinuousAnnotation(old::Annotation &from,
		old::Annotation &to,
		...
		old::Annotation::Entry *entry = 0;
		old::Annotation::Entry new_entry;
		...
		ssi_char_t delim) 
	{
		...
		for (size_t i = strlen(filepath) - 1; i >= 0; i--) {
			if (filepath[i] == SSI_FILE_SEPARATOR) {
	...
	void ModelTools::LoadSampleList(SampleList &sample_list, StringList &files)
	{
		...
		for (ssi_size_t i = 0; i < files.size(); i++) {
			...
			for (size_t j = strlen(filepath) - 1; j >= 0; j--) {
				if (filepath[j] == SSI_FILE_SEPARATOR) {
	...
	bool ModelTools::LoadSampleList(SampleList &samples,
		...
		while (sample = sin.next()) {
			sample->class_id = sample->class_id == SSI_SAMPLE_GARBAGE_CLASS_ID ? SSI_SAMPLE_GARBAGE_CLASS_ID : class_map[sample->class_id];
	...
	void ModelTools::SaveSampleList(ISamples &sample_list,
		...
		while (sample = sample_list.next()) {
			...
			ssi_sprint(filename, "%s%c%s%c%s%c%010ld%s", dir, SSI_FILE_SEPARATOR, user_id, delim, class_id, delim, timestamp, type);
	...
	void ModelTools::SaveSampleListArff(ISamples &sample_list,
		...
		if (file_name)
		{
			fprintf(file, "@attribute name string\n");
		}
		...
		fprintf(file, "@attribute %s {", class_attribute_name);
		...
		ssi_size_t count = 1;
		while (sample = sample_list.next()) {
			const char *class_id = sample->class_id == SSI_SAMPLE_GARBAGE_CLASS_ID ? "?" : sample_list.getClassName(sample->class_id);
			...
			if (sample->streams[0]->num != 0) {
				...
				if (file_name)
				{
					fprintf(file, "'%s_%04u.wav',", file_name, count++);
				}
	...
	void ModelTools::CopySampleList(ISamples &from, SampleList &to)
	{
		...
		while (sample = from.next())
		{
			...
			clone->class_id = clone->class_id == SSI_SAMPLE_GARBAGE_CLASS_ID ? SSI_SAMPLE_GARBAGE_CLASS_ID : new_label[sample->class_id];
	...
	void ModelTools::CreateTestSamples(SampleList &samples,
		...
		ssi_real_t *data = 0;
		Randomf random60(0, 60);
		ssi_time_t time = random60.next();		
		for (ssi_size_t n_class = 0; n_class < n_classes; n_class++) {
			ssi_real_t center_x = distr[n_class][0];
			ssi_real_t center_y = distr[n_class][1];
			ssi_real_t spread = distr[n_class][2];

			Randomf randomX(center_x, spread, Randomf::DISTRIBUTION::NORMAL);
			Randomf randomY(center_y, spread, Randomf::DISTRIBUTION::NORMAL);
	...
	void ModelTools::CreateDynamicTestSamples(SampleList &samples,
		...
		ssi_real_t *data = 0;
		Randomf random(0, 1);
		ssi_time_t time = random.next();
		Randomi randomMinMax(num_min, num_max);
		for (ssi_size_t n_class = 0; n_class < n_classes; n_class++) {
			ssi_real_t center_x = distr[n_class][0];
			ssi_real_t center_y = distr[n_class][1];
			ssi_real_t spread = distr[n_class][2];
			...
			for (ssi_size_t n_sample = 0; n_sample < n_samples_per_class; n_sample++) {
				ssi_stream_t **streams = new ssi_stream_t *[n_streams];
				ssi_size_t num = randomMinMax.next();
				for (ssi_size_t n_stream = 0; n_stream < n_streams; n_stream++) {
					streams[n_stream] = new ssi_stream_t;
	...
	void ModelTools::CreateMissingData(SampleList &samples, ssi_real_t prob) {
	...
	void ModelTools::CreateTestSamplesRegression(SampleList &samples,
		...
		const ssi_char_t* user)
	{
		...
		ssi_size_t user_id = samples.addUserName(user);				
		ssi_size_t class_id = samples.addClassName("REGRESSION");		
		...
		Randomf random(0, 1);
		ssi_time_t time = random.next();
		Randomf randomd(0, noise, Randomf::DISTRIBUTION::NORMAL);
		ssi_real_t value = 0;
		ssi_real_t bias = 0;
		for (ssi_size_t n = 0; n < n_samples; n++) {
			sample = new ssi_sample_t;			
			...
			bias = randomd.next();
			time += random.next();
			ssi_sample_create(*sample, 1, user_id, class_id, time, value);			
			stream = new ssi_stream_t;
			ssi_stream_init(*stream, 1, 1, sizeof(ssi_real_t), SSI_REAL, 10.0);
			*ssi_pcast(ssi_real_t, stream->ptr) = value + bias;
	...
	void ModelTools::PrintInfo(ISamples &samples, FILE *file) {
		if (!file)
		{
			ssi_print_off("samples:   %ux%u\n", samples.getSize(), samples.getStreamSize());
			ssi_print_off("per class: ");
			for (ssi_size_t nclass = 0; nclass < samples.getClassSize(); nclass++) {
				ssi_print("%u ", samples.getSize(nclass));
			}
			ssi_print("\n");
			ssi_print_off("names:     %u ( ", samples.getClassSize());
			for (ssi_size_t nclass = 0; nclass < samples.getClassSize(); nclass++) {
				ssi_print("%s ", samples.getClassName(nclass));
			}
			ssi_print(")\n");
			ssi_print_off("users:     %u ( ", samples.getUserSize());
			for (ssi_size_t nuser = 0; nuser < samples.getUserSize(); nuser++) {
				ssi_print("%s ", samples.getUserName(nuser));
			}
			ssi_print(")\n");
			ssi_print_off("streams:   %u ( ", samples.getStreamSize());
			for (ssi_size_t nstream = 0; nstream < samples.getStreamSize(); nstream++) {
				ssi_print("%u:%s ", samples.getStream(nstream).dim, SSI_TYPE_NAMES[samples.getStream(nstream).type]);
			}
			ssi_print(")\n");
		}
		else
	...
	void ModelTools::PlotSamples(ISamples &samples, const ssi_char_t *name, ssi_rect_t pos,
		ssi_size_t circle_size, ssi_rgb_t bg_color,	ssi_rgb_t circle_color,	ssi_size_t n_classes_colors, ssi_rgb_t classes_colors[])
		{
		...
		if (n_classes_colors > 0 || classes_colors != 0) {
			for (ssi_size_t i = 0; i < n_classes_colors; i++) {
				COLORS[i%7] = classes_colors[i];
			}
		}
		...
		for (ssi_size_t n_class = 0; n_class < n_classes; n_class++) {
			...
			plots[n_class]->setBackground(n_class == 0, bg_color);
			plots[n_class]->setPointSize((ssi_real_t) circle_size);
			plots[n_class]->setLimits(-0.2f, 1.2f);
			plots[n_class]->setData(stream, PaintData::TYPE::SCATTER);
			plots[n_class]->setBrush(COLORS[n_class]);
			plots[n_class]->setPen(circle_color);
	...
	void ModelTools::PlotSamplesRegression(ISamples &samples, const ssi_char_t *name, ssi_rect_t pos, ssi_rgb_t color,
		ssi_size_t circle_size, ssi_rgb_t bg_color, ssi_rgb_t circle_color)
	{
		PaintData plot;		
		Canvas canvas;
		...
		while (sample = samples.next()) {				
			for (ssi_size_t n_stream = 0; n_stream < n_streams; n_stream++) {						
				*data_ptr++ = *ssi_pcast(ssi_real_t, sample->streams[0]->ptr);
				*data_ptr++ = 1.0f - sample->score;						
			}			
		}

		plot.setBackground(true, bg_color);
		plot.setPointSize((ssi_real_t)circle_size);
		plot.setLimits(-0.2f, 1.2f);
		plot.setData(stream, PaintData::TYPE::SCATTER);
		plot.setBrush(color);
		plot.setPen(circle_color);
		canvas.addClient(&plot);
